...
int main (int argc, char *argv[]) {
  ...
  if (optionVerbose) {
    ...
    if (optionCount)
      printf ("Count ");
    if (optionRedInt)
      printf ("ReductionInterval ");
    if (optionRec)
      printf ("Recursive ");
    if (optionMultiThreaded)
  ...
  if (nbArgObl <= 1) {
    ...
    if (is_graphbin_file (fpPattern) && is_graphbin_file (fpToTest)) {
      ...
      if (wT == 0) {
        if (wP == 0) {
          ...
          for (i = 0; i < grPattern->nodes.count; i++) {
            ...
            if (vgraphP != NULL) {
              if (optionVerbose)
                printf ("processing site %i\n", i);
              nSgraphP++;

              // tries and find subgraph isomorphisms with
              // graphToTest :
              iso = isoUllman (vgraphP, grToTest);
              isoTotal += iso;
              // if (iso == 0) printf("site n°%d not
              // found.\n", i);
            }
          }
        ...
        if (wP == 0) {
          ...
          if (!optionIsoOnly) {
            ...
            for (k = 0; k < nThreads; k++) {
              ...
              struct structProcessGraph grData = { grPattern, borneThread[k],
                borneThread[k + 1],
                grToTest->nodes.count, vgraphsT
              };
            ...
            for (i = 0; i < grPattern->nodes.count; i++) {
              if (mP[i] != NULL) {
                ...
                for (j = 0; j < grToTest->nodes.count; j++) {
                  if (mT[j] != NULL) {
                    ...
                    if (pcharcmp (mP[i], mT[j], wP * wP)) {
                      ...
                      if (optionExport) {
                        // exports T graphSmall
                        // index
                        fprintf (FILEesT, "%d\n", j);
                        fflush (FILEesT);

                        // exports P graphSmall
                        // index
                        fprintf (FILEesP, "%d\n", i);
                        fflush (FILEesP);
                      }
                ...
                if (optionDebug && j == grToTest->nodes.count)
                  printf ("\nDEBUG %d No", i);
              }
            }
      ...
      if (grPattern->nodes.count >= 1)
        graph_free (grPattern);
      if (grToTest->nodes.count >= 1)
        graph_free (grToTest);
    }
    else {
