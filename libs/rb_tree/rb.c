...
rb_probe (struct rb_table *tree, void *item)
{
  ...
  n = pa[k - 1]->rb_link[da[k - 1]] = (struct rb_node*)
...
void
rb_t_init (struct rb_traverser *trav, const struct rb_table *tree)
{
  trav->rb_table = tree;
  trav->rb_node = NULL;
  trav->rb_height = 0;
...
void *
rb_t_first (struct rb_traverser *trav, const struct rb_table *tree)
{
  struct rb_node *x;
...
/* Initializes |trav| for |tree|
   ...
   Returns |NULL| if |tree| contains no nodes. */
void *
rb_t_last (struct rb_traverser *trav, const struct rb_table *tree)
{
  struct rb_node *x;
...
/* Searches for |item| in |tree|.
   ...
   and returns |NULL|. */
void *
rb_t_find (struct rb_traverser *trav, const struct rb_table *tree, void *item)
{
  struct rb_node *p, *q;
          ...
          rb_item_func *destroy, struct libavl_allocator *allocator)
{
  ...
  struct rb_table *new_rb;
  const struct rb_node *x;
  struct rb_node *y;

  assert (org != NULL);
  new_rb = rb_create (org->rb_compare, org->rb_param,
                    allocator != NULL ? allocator : org->rb_alloc);
  if (new_rb == NULL)
    return NULL;
  new_rb->rb_count = org->rb_count;
  if (new_rb->rb_count == 0)
    return new_rb;

  x = (const struct rb_node *) &org->rb_root;
  y = (struct rb_node *) &new_rb->rb_root;
  for (;;)
    {
      while (x->rb_link[0] != NULL)
        {
          ...
          y->rb_link[0] = (struct rb_node*)
            new_rb->rb_alloc->libavl_malloc (new_rb->rb_alloc,
                                           sizeof *y->rb_link[0]);
          if (y->rb_link[0] == NULL)
            {
              if (y != (struct rb_node *) &new_rb->rb_root)
                {
                  y->rb_data = NULL;
                  y->rb_link[1] = NULL;
                }

              copy_error_recovery (stack, height, new_rb, destroy);
              return NULL;
            }
